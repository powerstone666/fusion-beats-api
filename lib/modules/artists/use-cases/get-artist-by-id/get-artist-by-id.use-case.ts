...
import { createArtistPayload } from '@/lib/modules/artists/helpers'
import { HTTPException } from 'hono/http-exception'
import type { IUseCase } from '@/lib/common/types'
import type { ArtistAPIResponseModel, ArtistModel } from '@/lib/modules/artists/models'
...
export class GetArtistByIdUseCase implements IUseCase<GetArtistByIdArgs, z.infer<typeof ArtistModel>> {
  ...
  async execute({ artistId, page, songCount, albumCount, sortBy, sortOrder }: GetArtistByIdArgs) {
    const { data } = await saavnFetch<z.infer<typeof ArtistAPIResponseModel>>({

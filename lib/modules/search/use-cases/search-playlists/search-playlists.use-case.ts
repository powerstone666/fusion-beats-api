...
import { createSearchPlaylistPayload } from '@/lib/modules/search/helpers'
import { HTTPException } from 'hono/http-exception'
import type { IUseCase } from '@/lib/common/types'
import type { SearchPlaylistAPIResponseModel, SearchPlaylistModel } from '@/lib/modules/search/models'
...
export class SearchPlaylistsUseCase implements IUseCase<SearchPlaylistsArgs, z.infer<typeof SearchPlaylistModel>> {
  ...
  async execute({ query, limit, page }: SearchPlaylistsArgs): Promise<z.infer<typeof SearchPlaylistModel>> {
    const { data } = await saavnFetch<z.infer<typeof SearchPlaylistAPIResponseModel>>({

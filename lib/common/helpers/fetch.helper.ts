import { userAgents, type Endpoints } from '@/lib/common/constants'
import type { ApiContextEnum } from '@/lib/common/enums'
...
export const saavnFetch = async <T>({ endpoint, params, context }: FetchParams): Promise<FetchResponse<T>> => {
  ...
  const response = await fetch(url.toString(), {
    ...
    // Don't let a hung upstream hold a serverless function open.
